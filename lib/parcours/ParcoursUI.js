...
export class ParcoursUI {
  ...
  _handleResizeStart(e) {
    ...
    // Désactiver les événements sur l'iframe pendant le drag
    // (sinon l'iframe capture la souris et interrompt le resize)
    const iframe = this.el.viewer?.querySelector('.pv-slide-frame');
    if (iframe) {
  ...
  _handleResizeMove(e) {
    ...
    // Utiliser requestAnimationFrame pour fluidifier le rendu
    if (this._resizeRaf) {
  ...
  _handleResizeEnd() {
    ...
    // Réactiver les événements sur l'iframe
    const iframe = this.el.viewer?.querySelector('.pv-slide-frame');
    if (iframe) {
