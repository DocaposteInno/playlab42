...
export class ParcoursViewer {
  ...
  constructor(container, options = {}) {
    ...
    this._currentToc = null; // TOC injectée par la slide courante
  ...
  async load(epicId, slideId = null) {
    try {
      ...
      // Trouver l'epic
      const newEpic = catalogue.epics.find(e => e.id === epicId);
      if (!newEpic) { throw new Error(`Epic non trouvé: ${epicId}`); }

      // Détecter si on change d'epic (réinitialisation nécessaire)
      const epicChanged = this.epic && this.epic.id !== epicId;
      if (epicChanged) {
        // Réinitialiser les composants pour le nouvel epic
        this._progress = null;
        this.navigation = null;
        this.ui = null;
        this._currentIndexOverride = undefined;
      ...
      if (!this.ui) {
        ...
        // Configurer le handler pour les clics sur les ancres TOC
        this.ui.setTocAnchorClickHandler((anchorId) => {
  ...
  /**
   * Gère les messages postMessage des slides (iframe)
   * @param {MessageEvent} event
   */
  handleSlideMessage(event) {
    // Vérifier que le message vient de l'iframe slide
    const iframe = this.ui?.el?.slideFrame || this._el?.slideFrame;
    ...
    switch (type) {
      case 'slide:toc':
        this.handleSlideToc(items);
        break;
      case 'slide:toc:clear':
  ...
  /**
   * Traite la TOC reçue d'une slide
  ...
  handleSlideToc(items) {
    ...
    // Limites : max 15 items, niveaux 1-2
    const MAX_ITEMS = 15;
    const validItems = items
      .slice(0, MAX_ITEMS)
      .filter(item => {
        if (!item.id || !item.label) {
          console.warn('slide:toc - item invalide (id et label requis)', item);
          return false;
        }
        // Limiter les niveaux à 1-2
        if (item.level && (item.level < 1 || item.level > 2)) {
          item.level = Math.min(2, Math.max(1, item.level));
        }
        return true;
      })
      .map(item => ({
    ...
    // Injecter dans le menu via ParcoursUI
    if (this.ui?.injectSlideToc) {
      const currentSlide = this.slides[this.currentIndex];
      this.ui.injectSlideToc(currentSlide?.id, validItems);
    }
  }

  /**
   * Efface la TOC courante
   */
  clearCurrentToc() {
    this._currentToc = null;
    if (this.ui?.clearSlideToc) {
      const currentSlide = this.slides[this.currentIndex];
      this.ui.clearSlideToc(currentSlide?.id);
    }
  }

  /**
   * Envoie un message à la slide courante (iframe)
  ...
  /**
   * Demande à la slide de scroller vers une ancre
   * @param {string} anchorId - ID de l'ancre
   */
  scrollToAnchor(anchorId) {
    this.sendToSlide('viewer:scroll-to', { anchor: anchorId });
    // Mettre à jour l'ancre active dans le menu
    if (this.ui?.setActiveTocAnchor) {
  ...
  onSlideChange(slide, index) {
    // Effacer la TOC de la slide précédente
    this.clearCurrentToc();
    this.ui?.updateUI?.();
  ...
  setupEventListeners() {
    ...
    window.addEventListener('message', this.handleSlideMessage);
  ...
  close() {
    ...
    window.removeEventListener('message', this.handleSlideMessage);
